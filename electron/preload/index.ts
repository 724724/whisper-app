...
const api = {
  ...
  saveRealtimeProject: (params: {
    ...
    ipcRenderer.invoke('project:save-realtime', params),
  ...
  translateText: (params: { text: string; targetLang: string }): Promise<{ success: boolean; text?: string; error?: string }> =>
  ...
  moveToFolder: (params: { projectIds: string[]; folderId: string | null }): Promise<{ success: boolean }> =>
    ipcRenderer.invoke('project:move-to-folder', params),
