...
export function AppShell({ children }: AppShellProps) {
  ...
  const isRealtimePage = location.pathname === '/realtime'
  const showBack = isProjectPage || isRealtimePage
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      ...
      <SettingsModal />
      <ModelManagerDialog open={modelManagerOpen} onClose={() => setModelManagerOpen(false)} />
    </Box>
  )
}
